...
run_bbot() {
    ...
    if [ -d "$output_dir/subdomains/bbot" ]; then
        find "$output_dir/subdomains/bbot" -name "*.txt" -exec cat {} \; 2>/dev/null |             grep -E "^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$" | sort -u > "$output_dir/subdomains/bbot/bbot_subdomains.txt" ||             touch "$output_dir/subdomains/bbot/bbot_subdomains.txt"
    ...
    while read -r domain; do
        if [ ! -z "$domain" ]; then
            ...
            if [ -f "$output_dir/subdomains/ffuf/ffuf_${domain//[.:]/_}.json" ]; then
                jq -r '.results[].url' "$output_dir/subdomains/ffuf/ffuf_${domain//[.:]/_}.json" 2>/dev/null |                     sed 's|https\?://||' | cut -d'/' -f1 >> "$output_dir/subdomains/ffuf/ffuf.txt" || true
            fi
        fi
    done < "$targets_file"
...
run_subdog() {
    ...
    # Use exact command format from user's file
    cat "$targets_file" | subdog -tools all > "$output_dir/subdomains/subdog/subdog.txt" 2>/dev/null ||         touch "$output_dir/subdomains/subdog/subdog.txt"
    ...
    while read -r domain; do
        if [ ! -z "$domain" ]; then
            ...
            if [ -d "$output_dir/subdomains/sudomy/individual/sudomy_${clean_domain}" ]; then
                find "$output_dir/subdomains/sudomy/individual/sudomy_${clean_domain}" -name "*.txt" -exec cat {} \; 2>/dev/null |                     grep -E "^[a-zA-Z0-9.-]+\.$domain$" >> "$output_dir/subdomains/sudomy/sudomy.txt" || true
            fi
        fi
    done < "$targets_file"
...
run_dnscan() {
    ...
    # Use exact command format from user's file
    dnscan -l "$targets_file" -w "$wordlist" -r --maxdepth 3 -o "$output_dir/subdomains/dnscan/dnscan.txt" 2>/dev/null ||         touch "$output_dir/subdomains/dnscan/dnscan.txt"
